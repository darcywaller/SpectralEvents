...
def test_event_comparison(method):
    ...
    mat_all_evs = matlab_file['event_times'][0:200, 0]  # evs from 1st subject

    # Get python events by running find_events on demo data
    # <----------SIMPLIFY BY GETTING PICKLE FILES FOR THIS INSTEAD----------->
    fname = op.join(data_dir, 'data',
                    'prestim_humandetection_600hzMEG_subject1.mat')
    ...
    # find events
    py_all_evs = se.find_events(tfr=tfrs, times=times, freqs=freqs, event_band=event_band,
                              thresholds=None, threshold_FOM=thresh_fom, find_method=method)

    # put from cell array/dict into sets
    py_evs = np.array([len(trial) for trial in py_all_evs])
